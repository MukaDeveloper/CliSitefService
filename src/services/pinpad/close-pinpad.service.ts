...
export default class ClosePinpad extends BaseService {
    ...
    try {
      ...
      if (axiosError?.response) {
        ...
        if (isErrorWithMessage(axiosError?.response.data)) {
          message = `Erro ao fechar pinpad: ${axiosError?.response.data.message}`;
          console.error(message);
        } else {
          /**
           * Se não for um objeto com 'message', apenas stringify o que quer que seja.
           */
          message = `Erro ao fechar pinpad: ${JSON.stringify(
        ...
        console.error(
          `Nenhuma resposta do servidor ao fechar pinpad: ${axiosError?.message}`
        );
      } else {
        console.error(
          `Erro ao configurar a requisição ao fechar pinpad: ${axiosError?.message}`
        );
      }
