...
export default class IsPresentPinpad extends BaseService {
  ...
  public async execute(sessionId: string) {
    try {
      const res = await axios.post<any>(
        this.agenteUri + "/pinpad/isPresent",
        { sessionId },
        {
      ...
      if (axiosError?.response) {
        ...
        if (isErrorWithMessage(axiosError?.response.data)) {
          message = `Erro ao verificar pinpad: ${axiosError?.response.data.message}`;
          console.error(message);
        } else {
          /**
           * Se não for um objeto com 'message', apenas stringify o que quer que seja.
           */
          message = `Erro ao verificar pinpad: ${JSON.stringify(
        ...
        console.error(
          `Nenhuma resposta do servidor ao verificar pinpad: ${axiosError?.message}`
        );
      } else {
        console.error(
          `Erro ao configurar a requisição ao verificar pinpad: ${axiosError?.message}`
        );
      }
