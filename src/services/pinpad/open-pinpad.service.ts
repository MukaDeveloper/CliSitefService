...
export default class OpenPinpad extends BaseService {
    ...
    try {
      ...
      if (axiosError?.response) {
        ...
        if (isErrorWithMessage(axiosError?.response.data)) {
          message = `Erro ao abrir pinpad: ${axiosError?.response.data.message}`;
          console.error(message);
        } else {
          /**
           * Se não for um objeto com 'message', apenas stringify o que quer que seja.
           */
          message = `Erro ao abrir pinpad: ${JSON.stringify(
        ...
        console.error(
          `Nenhuma resposta do servidor ao abrir pinpad: ${axiosError?.message}`
        );
      } else {
        console.error(
          `Erro ao configurar a requisição ao abrir pinpad: ${axiosError?.message}`
        );
      }
