...
export default class ReadYesNoPinpad extends BaseService {
   ...
   */
  public async execute(sessionId: string, message: string) {
    try {
      const res = await axios.post<any>(
        this.agenteUri + "/pinpad/readYesNo",
        { sessionId, message },
        {
      ...
      if (axiosError?.response) {
        ...
        if (isErrorWithMessage(axiosError?.response.data)) {
          message = `Erro ao ler tecla do pinpad: ${axiosError?.response.data.message}`;
          console.error(message);
        } else {
          /**
           * Se não for um objeto com 'message', apenas stringify o que quer que seja.
           */
          message = `Erro ao ler tecla do pinpad: ${JSON.stringify(
        ...
        console.error(
          `Nenhuma resposta do servidor ao ler tecla do pinpad: ${axiosError?.message}`
        );
      } else {
        console.error(
          `Erro ao configurar a requisição ao ler tecla do pinpad: ${axiosError?.message}`
        );
      }
