...
export default class DeleteSession extends BaseService {
    ...
    try {
      ...
      if (axiosError?.response) {
        ...
        if (isErrorWithMessage(axiosError?.response.data)) {
          message = `Erro ao deletar sessão: ${axiosError?.response.data.message}`;
          console.error(message);
        } else {
          /**
           * Se não for um objeto com 'message', apenas stringify o que quer que seja.
           */
          message = `Erro ao deletar sessão: ${JSON.stringify(
        ...
        console.error(
          `Nenhuma resposta do servidor ao deletar sessão: ${axiosError?.message}`
        );
      } else {
        console.error(
          `Erro ao configurar a requisição ao deletar sessão: ${axiosError?.message}`
        );
      }
