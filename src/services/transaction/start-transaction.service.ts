...
export default class StartTransaction extends BaseService {
	...
	public async execute(transaction: IStartTransaction): Promise<IStartTransactionResponse> {
		/**
		...
		try {
			 ...
			 */
			const res = await axios.post<IStartTransactionResponse>(this.agenteUri + "/startTransaction", qs.stringify(transaction), {
			...
			return {
				serviceStatus: 1,
				serviceMessage: axiosError?.message || error?.message || "Erro desconhecido",
				clisitefStatus: 0,
			};
		}
	}
